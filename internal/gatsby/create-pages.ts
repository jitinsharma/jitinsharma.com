...
const createPages: GatsbyNode["createPages"] = async ({ graphql, actions }) => {
  ...
  pages.forEach((edge) => {
    ...
    } else if (node?.frontmatter?.template === "talks" && node?.fields?.slug) {
      createPage({
        path: node?.frontmatter?.slug || node.fields.slug,
        component: templates.talksTemplate,
        context: { slug: node.fields.slug },
      });
    ...
    } else if (node?.frontmatter?.template === "presentation" && node?.fields?.slug) {
      createPage({
        path: node?.frontmatter?.slug || node.fields.slug,
        component: templates.presentationTemplate,
        context: { slug: node.fields.slug },
      });
    }
  });
